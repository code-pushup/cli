import { getProfiler } from '../../src/index.js';

// 1. Generate output:
// npx tsx ./example/exit-uncaught-exception.ts
// 2. Evaluate output:
/*
node -e "
const out = require(\"./tmp/profiles/exit-uncaught-exception.json\");
const contentCheck = [
  {\"cat\":\"devtools.timeline\", \"name\":\"TracingStartedInBrowser\"},
  {\"cat\":\"devtools.timeline\", \"name\":\"RunTask\"},
  {\"cat\":\"blink.user_timing\", \"ph\":\"I\",\"name\":\"buffered-mark:start\"},
  {\"cat\":\"blink.user_timing\", \"ph\":\"I\",\"name\":\"mark:start\"},
  {\"cat\":\"blink.user_timing\",\"s\":\"t\",\"ph\":\"I\",\"name\":\"PROCESS:FATAL-ERROR: Error\"},
  {\"cat\":\"devtools.timeline\",\"ph\":\"X\",\"name\":\"RunTask\"}
];
contentCheck.map(expected => {
  const found = out.traceEvents?.find(event =>
    Object.keys(expected).every(key => event[key] === expected[key])
  );
  return found ? \"✓\" : \"✗\";
});
"
*/

performance.mark('buffered-mark:start');

const profiler = getProfiler({
  enabled: true,
  fileName: 'exit-uncaught-exception',
});

profiler.mark('mark:start');

throw new Error('Uncaught exception example');
