...
export class Logger {
  ...
  #ciPlatform: CiPlatform | undefined = isEnvVarEnabled('GITHUB_ACTIONS') ? 'GitHub' : isEnvVarEnabled('GITLAB_CI') ? 'GitLab' : undefined;
  ...
  #sigintListener = () => {
    ...
    process.exit(os.platform() === 'win32' ? SIGINT_EXIT_CODE_WINDOWS : SIGINT_EXIT_CODE_UNIX);
  };
   ...
   */
  async task<T = undefined>(title: string, worker: () => Promise<string | { message: string; result: T }>): Promise<T> {
  ...
  async group<T = undefined>(title: string, worker: () => Promise<string | { message: string; result: T }>): Promise<T> {
    if (this.#groupColor) {
    ...
    if (result.status === 'fulfilled') {
      const message = typeof result.value === 'string' ? result.value : result.value.message;
      console.log([this.#colorize(this.#groupSymbols.end, this.#groupColor), this.#colorize(message, 'green'), this.#formatDurationSuffix({ start, end })].join(' '));
    } else {
      console.log([this.#colorize(this.#groupSymbols.end, this.#groupColor), this.#colorize(`${stringifyError(result.reason, { oneline: true })}`, 'red')].join(' '));
    }
  ...
  #formatGroupTitle(title: string, symbols: { prefix: boolean }): string {
    const text = symbols.prefix ? `${this.#groupSymbols.start} ${title}` : title;
  ...
  async #spinner<T>(
    ...
    const text = result.status === 'fulfilled' ? [messages.success(result.value), this.#formatDurationSuffix({ start, end })].join(' ') : messages.failure(stringifyError(result.reason, { oneline: true }));
  ...
  #log(message: string, color?: AnsiColors, options?: LogOptions): void {
    const print: (text: string) => void = options?.noLineBreak ? text => process.stdout.write(text) : console.log;

    if (this.#activeSpinner) {
      if (this.#activeSpinner.isSpinning) {
        this.#activeSpinnerLogs.push(this.#format(message, color));
      } else {
        const indented = options?.noIndent || !message ? message : indentLines(message, 2);
  ...
  #formatDurationSuffix({ start, end }: { start: number; end: number }): string {
...
Extensibility API trace example:
...
```json
{
  "traceEvents": [
    ...
    {
      "args": {
        "detail": "{\"devtools\":{\"dataType\":\"track-entry\",\"color\":\"secondary\",\"trackGroup\":\"Custom track timings\",\"track\":\"Custom timing A\",\"properties\":[[\"Description\",\"step-A1\"],[\"Duration (ms)\",\"10.00\"]],\"tooltipText\":\"step-A1 took 10.00ms\"}}"
      },
    ...
    {
      "args": {
        "detail": "{\"devtools\":{\"dataType\":\"track-entry\",\"color\":\"secondary\",\"trackGroup\":\"Custom track timings\",\"track\":\"Custom timing B\",\"properties\":[[\"Description\",\"step-B1\"],[\"Duration (ms)\",\"12.00\"]],\"tooltipText\":\"step-B1 took 12.00ms\"}}"
      },
      "cat": "blink.user_timing",
      "id2": { "local": "0xB1" },
      "name": "step-B1",
    ...
    {
      "args": {
        "detail": "{\"devtools\":{\"dataType\":\"track-entry\",\"color\":\"secondary\",\"trackGroup\":\"Custom track timings\",\"track\":\"Custom timing C\",\"properties\":[[\"Description\",\"step-C1\"],[\"Duration (ms)\",\"15.00\"]],\"tooltipText\":\"step-C1 took 15.00ms\"}}"
      },
      "cat": "blink.user_timing",
      "id2": { "local": "0xC1" },
      "name": "step-C1",
    ...
    {
      "args": {
        "detail": "{\"devtools\":{\"dataType\":\"track-entry\",\"color\":\"secondary\",\"trackGroup\":\"Custom track timings\",\"track\":\"Custom timing A\",\"properties\":[[\"Description\",\"step-1\"],[\"Duration (ms)\",\"50.00\"]],\"tooltipText\":\"step-1 took 50.00ms\"}}"
      },
      "cat": "blink.user_timing",
      "id2": { "local": "0x2" },
      "name": "step-1",
    ...
    {
      "args": {
        "detail": "{\"devtools\":{\"dataType\":\"track-entry\",\"color\":\"secondary\",\"trackGroup\":\"Custom track timings\",\"track\":\"Custom timing B\",\"properties\":[[\"Description\",\"step-2\"],[\"Duration (ms)\",\"30.00\"]],\"tooltipText\":\"step-2 took 30.00ms\"}}"
      },
      "cat": "blink.user_timing",
      "id2": { "local": "0x2" },
      "name": "step-2",
    ...
    {
      "args": {
        "detail": "{\"devtools\":{\"dataType\":\"track-entry\",\"color\":\"secondary\",\"trackGroup\":\"Custom track timings\",\"track\":\"Custom timing C\",\"properties\":[[\"Description\",\"step-3\"],[\"Duration (ms)\",\"10.00\"]],\"tooltipText\":\"step-3 took 10.00ms\"}}"
      },
      "cat": "blink.user_timing",
      "id2": { "local": "0x2" },
      "name": "step-3",
