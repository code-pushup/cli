...
const config: CoreConfig = {
  ...
  plugins: [
    await eslintPlugin(await eslintConfigFromNxProject('plugin-lighthouse')),
    await jsPackagesPlugin({
      packageManager: 'npm',
      packageJsonPath: join('packages', 'plugin-lighthouse', 'package.json'),
    }),
    await coveragePlugin({
      ...
      reports: ['unit-tests', 'integration-tests'].map(target =>
        join('coverage', 'plugin-lighthouse', target, 'lcov.info'),
      ),
    }),
  ],
