...
describe('settlePromise', () => {
  it('should wrap resolved value in object with status (as in `Promise.allSettled`)', () => {
    expect(settlePromise(Promise.resolve(42))).resolves.toEqual({
      status: 'fulfilled',
      value: 42,
    });
  });

  it('should resolve rejected promise', () => {
    const error = new Error('something went wrong');
    expect(settlePromise(Promise.reject(error))).resolves.toEqual({
      status: 'rejected',
      reason: error,
    });
  });
