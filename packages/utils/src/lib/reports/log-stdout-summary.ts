...
function reportToHeaderSection(report: ScoredReport): string {
  ...
  return `${pc.bold(reportHeadlineText)} - ${packageName}@${version}`;
}
...
function logPlugins(report: ScoredReport): void {
  ...
  plugins.forEach(plugin => {
    ...
    log();
    log(pc.bold(pc.magentaBright(`${title} audits`)));
    log();
    audits.forEach((audit: AuditReport) => {
      ui().row([
        ...
        {
          text: pc.cyanBright(audit.displayValue || `${audit.value}`),
          width: 10,
          padding: [0, 0, 0, 0],
        },
      ]);
...
function logCategories({ categories, plugins }: ScoredReport): void {
  ...
  log(pc.bold(pc.magentaBright('Categories')));
  log();
  table.render();
  log();
}
...
function applyScoreColor({ score, text }: { score: number; text?: string }) {
  ...
  if (score >= SCORE_COLOR_RANGE.GREEN_MIN) {
    return text ? pc.green(formattedScore) : pc.bold(pc.green(formattedScore));
  }

  if (score >= SCORE_COLOR_RANGE.YELLOW_MIN) {
    return text
      ? pc.yellow(formattedScore)
      : pc.bold(pc.yellow(formattedScore));
  }

  return text ? pc.red(formattedScore) : pc.bold(pc.red(formattedScore));
}
