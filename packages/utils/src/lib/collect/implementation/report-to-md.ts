...
import {countWeightedRefs, sumRefs, reportHeadlineText} from './utils';
...
function reportToMetaSection(report: Report, config: CoreConfig): string {
  ...
  return `---` + NEW_LINE +
    ...
    `_Duration: ${duration}ms_` + NEW_LINE +
...
function reportToOverviewSection(report: Report, config: CoreConfig): string {
  ...
  const tableContent: string[][] = [
    ...
    ...categories.map(({
                         title,
                         refs
                       }) => ([title, sumRefs(refs).toString(), refs.length.toString() + '/' + countWeightedRefs(refs)]))
  ];
      ...
      .map(({slug: auditSlugInCategoryRefs, weight, plugin: pluginSlug}) => {
        const audit =
          plugins
            ...
            ?.audits.find(({slug: auditSlugInPluginAudits}) => auditSlugInPluginAudits === auditSlugInCategoryRefs);
