...
export async function collectAndPersistReports(
  ...
  const sortedScoredReport = profiler.measure(
    ...
    },
    {
      ...
      success: (sortedScoredReport: ReturnType<typeof sortReport>) => {
        ...
        return {
          properties: [
            ['Categories', String(sortedScoredReport.categories?.length || 0)],
  ...
  if (skipReports) {
    ...
    const reportFiles = await profiler.measureAsync(
      ...
      {
        color: 'primary',
        success: (reportFiles: Awaited<ReturnType<typeof persistReport>>) => ({
          properties: [
