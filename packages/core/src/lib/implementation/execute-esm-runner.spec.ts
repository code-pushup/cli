...
import {AuditOutputs, auditOutputsSchema, EsmObserver,} from '@code-pushup/models';
...
describe('executeEsmRunner', () => {
  it('should execute valid plugin config', async () => {
    const autidOutputs = await executeEsmRunner({
      ...
      runner: (observer?: EsmObserver) => Promise.resolve([{slug: 'mock-audit-slug', score: 0, value: 0}] satisfies AuditOutputs)
    });
