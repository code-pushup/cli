  ...
  categories: z.array(categoryConfigSchema, {
    ...
    .refine(
      ...
      (categoryCfg) => {
        ...
        const duplicateStringSlugs = duplicateStrings !== true ? duplicateStrings.join(', ') : '';
        return {
    ...
    .refine(
      categoryCfg => stringsUnique(categoryCfg.flatMap(({metrics}) => metrics.map(({ref}) => ref))) === true,
      (categoryCfg) => {
        const duplicateStrings = stringsUnique(
          categoryCfg.flatMap(({metrics}) => metrics.map(({ref}) => ref))
        );
        const duplicateStringRefs = duplicateStrings !== true ? duplicateStrings.join(', ') : '';
        return {
