  ...
  describe('CategoryConfig', () => {
    ...
    it('should throw if audit refs are not unique', () => {
      ...
      cfg.categories.push(
        mockCategory({
          categorySlug: 'test',
          auditRef: [duplicatedSlug, duplicatedSlug],
        }),
      );
      expect(() => coreConfigSchema.parse(cfg)).toThrow(
        `In the categories, the following audit ref's are duplicates: ${duplicatedSlug}`,
      );
    });
    ...
    it('should throw if ref in a category does not exist in plugins', () => {
      ...
      cfg.categories.push(
        mockCategory({ categorySlug: 'test', auditRef: [`${missingSlug}`] }),
      );
      expect(() => coreConfigSchema.parse(cfg)).toThrow(
        `In the categories, the following plugin ref's do not exist in the provided plugins: ${missingSlug}`,
      );
    });
  ...
  */
  describe('PluginConfigGroup', () => {
    ...
    it('should throw if group does not point to an audit', () => {
      ...
      expect(() => coreConfigSchema.parse(cfg)).toThrow(
        `In the categories, the following plugin ref's do not exist in the provided plugins: ${missingRef}`,
      );
    });
  });
