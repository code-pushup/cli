...
const config: CoreConfig = {
  ...
  plugins: [
    await eslintPlugin(await eslintConfigFromNxProject('plugin-coverage')),
    await jsPackagesPlugin({
      packageManager: 'npm',
      packageJsonPath: join('packages', 'plugin-coverage', 'package.json'),
    }),
    await coveragePlugin({
      ...
      reports: ['unit-tests', 'integration-tests'].map(target =>
        join('coverage', 'plugin-coverage', target, 'lcov.info'),
      ),
    }),
  ],
